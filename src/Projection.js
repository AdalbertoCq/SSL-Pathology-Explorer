...
let sprite_number = 24
let sprite_image_size = 112
// actual sprite size needs to be power of 2
let sprite_actual_size = 2240

let mnist_tile_string = 'mnist_tile_solid_'
let mnist_tile_locations = [...Array(sprite_number)].map( (n, i) => `${process.env.PUBLIC_URL}/${mnist_tile_string}${i}.png` )
...
class Projection extends Component {
  ...
  addPoints() {
    ...
    let embedding_chunks = ranges.map(range => mnist_embeddings.slice(range[0], range[1]) )
    let label_chunks = ranges.map(range => mnist_labels.slice(range[0], range[1]) )
